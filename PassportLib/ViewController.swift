...
class ViewController: UIViewController {
    ...
    @IBAction func startPress(_ sender: UIButton) {
        ...
        lib?.ReadRFIDData(mrz:mrz,dg1: true,dg2: false,dg3: false,dg11: true,dg12:true,dg15:true)

    }
    
    @IBAction func pressShowData(_ sender: Any) {
        textArea.text = lib?.model?.DG1
        if lib?.model?.faceImage != nil {
            let data = Data(base64Encoded: (lib?.model?.faceImage)!)
        ...
        print(lib?.model?.personalNumber!)
        print(lib?.model?.fullDateOfBirth!)
        print(lib?.model?.placeOfBirth!)
        print(lib?.model?.permanentAddress!)
        print(lib?.model?.title!)
